...
In several of the datasets, there are clear outliers that would strongly influence the models we built. Our approach was to use the `forecast::tsclean` function to resolve this issue. We use tsclean function to return a cleaned version of a time series with outliers and missing values replaced by estimated values. This function internally calls na_interpolation and replaces the outliers with the estimated values, if we select the parameter `replace.missing = True`  then it interpolates missing values as well.
...
```{r, warning=F}
S02 <- imputeTS::na_interpolation(S02)
...
Reviewing our results above, we can see that the top plot looks much more like white noise, additionally we see that we have less of our spikes crossing the critical threshold. This data is now stationary.
